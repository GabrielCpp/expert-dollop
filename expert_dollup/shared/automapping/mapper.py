...
class Mapper:
    ...
    def add_mapping(self, from_type: Type[T], to_type: Type[U], mapping_function: Callable[[T, Optional['Mapper']], U]):
        self._mappings[from_type][to_type] = mapping_function

    def map_many(self, instances: List[T], to_type: Type[U], from_type: Type[T] = None, after=lambda x: x) -> List[U]:
        ...
        for module in modules:
            ...
            for (member_name, *other) in members:
                ...
                if isfunction(member) and member.__name__.startswith('map_'):
                    ...
                    from_type = next(
                        sig.parameters.values().__iter__(), None).annotation
