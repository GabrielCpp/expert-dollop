...
from expert_dollup.core.exceptions import RessourceNotFound, InvalidObject, FactorySeedMissing
from expert_dollup.core.domains import ProjectDefinitionContainer, ProjectDefinition
from expert_dollup.infra.services import ProjectDefinitionContainerService, ProjectDefinitionService, ProjectDefinitionPluginService
...
class ProjectDefinitonContainerUseCase:
    ...
    async def _ensure_container_is_valid(self, domain: ProjectDefinitionContainer):
        project_def = await self.project_container_service.find_by_id(domain.project_def_id)
    ...
    async def _ensure_plugin_config_valid(self, project_def: ProjectDefinition, domain: ProjectDefinitionContainer) -> None:
        schema_by_name = await self.project_definition_plugin_service.get_config_validation_schemas(
            project_def.plugins)
